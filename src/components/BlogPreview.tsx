          ...
          {latestPosts.map((post, i) => (
            <Link key={post.slug} href={`/blog/${post.slug}`}>
              <motion.article
                ...
                className="group bg-ink-900 border border-ink-600 rounded-sm card-hover shadow-md transition-all duration-700 overflow-hidden"
              >
                {/* Image */}
                <div className="relative h-72 overflow-hidden">
                  <Image 
                    ...
                    width={600}
                    height={400}
                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 opacity-85" 
                  />
                  <div className="absolute top-6 left-6">
                    <span className="px-5 py-2 bg-ink-950/80 backdrop-blur-md text-gold-400 text-[10px] uppercase tracking-[0.2em] font-bold border border-gold-400/20 rounded-sm">
                      {post.category}
                    </span>
                  </div>
                </div>
                <div className="p-8">
                  ...
                  <div className="mt-auto flex items-center gap-3 text-gold-400 text-[11px] uppercase tracking-[0.3em] font-bold group-hover:gap-5 transition-all">
                    Read Article <ArrowRight size={16} className="text-gold-400" />
                  </div>
                </div>
              </motion.article>
            </Link>
