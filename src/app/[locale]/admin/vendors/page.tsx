    ...
    <div className="pb-20 max-w-7xl mx-auto">
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div>
          ...
          <p className="text-sand-300 font-medium">Manage your professional network of elite event service providers.</p>
        </div>
        <button 
      ...
      <div className="bg-ink-800 border border-ink-600 rounded-[2.5rem] shadow-sm overflow-hidden">
        ...
        <div className="p-8 border-b border-ink-500 flex flex-col lg:flex-row gap-10 items-end bg-ink-950/30">
          ...
          <div className="flex flex-wrap items-end gap-10 w-full lg:w-auto">
            ...
            <button 
              ...
              disabled={loading}
              className={`p-3 bg-ink-800 border border-ink-600 rounded-2xl text-gold-600 hover:bg-ink-950 transition-all shadow-sm disabled:opacity-50`}
            >
          ...
          {loading ? (
            ...
            <div className="text-center py-32">
              <div className="w-20 h-20 bg-ink-950 rounded-full flex items-center justify-center mx-auto mb-8">
                <Briefcase size={32} className="text-sand-500" />
              </div>
              <p className="text-sand-300 font-medium tracking-tight">No vendors found matching your criteria.</p>
              {(search || category !== 'all') && (
                <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {vendors.map((vendor: any, i: number) => (
                <motion.div
                  ...
                  className="bg-ink-800 border border-ink-500 rounded-[2rem] p-8 hover:border-gold-500/30 hover:shadow-2xl hover:shadow-gold-500/5 transition-all duration-500 group relative flex flex-col"
                >
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-1.5 bg-ink-950 px-3 py-1.5 rounded-xl border border-ink-500">
                      <Star className="text-gold-500 fill-gold-500" size={14} />
                      <span className="text-xs text-sand-100 font-black">{vendor.rating.toFixed(1)}</span>
                    </div>
                  </div>
                  
                  <div className="mb-8 flex-1">
                    <h3 className="text-lg font-black text-sand-50 mb-1 group-hover:text-gold-600 transition-colors tracking-tight">{vendor.name}</h3>
                    <p className="text-[10px] text-sand-400 uppercase font-black tracking-[0.25em] mb-8">{vendor.category}</p>
                    <p className="text-xs text-sand-300 font-medium line-clamp-2 leading-relaxed italic">
                      {vendor.services || 'Premium services tailored for luxury events.'}
                    </p>
                  </div>
                  
                  <div className="pt-6 border-t border-slate-50 flex items-center justify-between text-sand-400">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black uppercase tracking-widest mb-0.5">Engagement</span>
                      <span className="text-xs text-sand-50 font-black">{vendor._count?.events || 0} Events</span>
                    </div>
                    <button className="w-10 h-10 rounded-xl hover:bg-ink-950 flex items-center justify-center transition-all group-hover:text-sand-50">
            ...
            >
              <div className="p-8 border-b border-ink-500 flex items-center justify-between bg-ink-950/30">
                <div className="flex items-center gap-10">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-sand-50 tracking-tight">
                      Initialize Partner
                    </h2>
                    <p className="text-[10px] text-sand-400 font-bold uppercase tracking-widest mt-0.5">Network Expansion</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsModalOpen(false)}
                  className="p-2 text-sand-400 hover:text-sand-50 hover:bg-ink-800 rounded-xl transition-all shadow-sm border border-transparent hover:border-ink-500"
                >
                  <X size={22} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <form onSubmit={handleAddVendor} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-widest text-sand-400 px-1">Business Identity</label>
                    <input 
                      type="text" 
                      required
                      value={formData.name}
                      ...
                      className="w-full bg-ink-950 border border-ink-600 rounded-xl py-3 px-4 text-sm font-medium text-sand-50 focus:outline-none focus:ring-4 focus:ring-gold-500/5 focus:border-gold-500 transition-all placeholder:text-sand-500"
                    />
                  </div>

                  <div className="grid grid-cols-2 gap-10">
                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-sand-400 px-1">Category</label>
                      <select 
                        required
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-black uppercase tracking-widest text-sand-400 px-1">Initial Rating</label>
                      <input 
                        type="number" 
                        step="0.1" 
                        min="1.0" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-widest text-sand-400 px-1">Contact Metadata (Email/Phone)</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase tracking-widest text-sand-400 px-1">Service Portfolio Summary</label>
                    <textarea 
                      required
                      rows={3}
                      ...
                      className="w-full bg-ink-950 border border-ink-600 rounded-xl py-3 px-4 text-sm font-medium text-sand-50 focus:outline-none focus:ring-4 focus:ring-gold-500/5 focus:border-gold-500 transition-all placeholder:text-sand-500 resize-none"
                    />
                  </div>
