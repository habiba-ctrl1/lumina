    ...
    <div className="pb-20 max-w-7xl mx-auto">
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-10">
        ...
        <div className="flex flex-wrap items-center gap-10">
          <div className="flex items-center gap-3 bg-ink-800 px-4 py-2 rounded-2xl border border-ink-600 shadow-sm">
            <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
            <span className="text-xs font-bold text-sand-200 uppercase tracking-wider">System Live</span>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
        {statCards.map((card, i) => (
          <motion.div
            ...
            className="bg-ink-800 border border-ink-600 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 group"
          >
            ...
            <div>
              <p className="text-xs font-semibold text-sand-300 mb-1">{card.label}</p>
              <h3 className="text-2xl font-bold text-sand-50">
                {loading ? "—" : card.value}
              </h3>
              <p className="text-[11px] text-sand-400 mt-2 font-medium">{card.subtext}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
        <div className="bg-ink-800 border border-ink-600 rounded-2xl p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
              <BarChart3 size={18} />
            </div>
            <div>
              <h2 className="text-base font-bold text-sand-50 tracking-tight">Event Pipeline</h2>
              <p className="text-xs text-sand-300 font-medium">Distribution of events by stage</p>
            </div>
          </div>
          <div className="h-72 w-full flex items-center justify-center">
            {!loading && pipelineData.every(d => d.value === 0) ? (
              <div className="text-center">
                ...
                <p className="text-xs text-sand-400 font-medium tracking-wide">No events yet. Start by adding your first event.</p>
              </div>
            ) : (
        ...
        <div className="bg-ink-800 border border-ink-600 rounded-2xl p-8 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h2 className="text-base font-bold text-sand-50 tracking-tight">Lead Sources</h2>
              <p className="text-xs text-sand-300 font-medium">Where your inquiries are coming from</p>
            </div>
          </div>
          <div className="h-72 w-full flex items-center justify-center">
            {!loading && sourceData.length === 0 ? (
              <div className="text-center">
                ...
                <p className="text-xs text-sand-400 font-medium tracking-wide">No lead data yet</p>
              </div>
            ) : (
              <>
                  ...
                  {sourceData.map((item, i) => (
                    <div key={item.name} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-sand-200">{item.name}</span>
                      </div>
                      <span className="text-xs font-black text-sand-50">{item.value}%</span>
                    </div>
                  ))}
                </div>
              </>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Quotes Section */}
          <div className="bg-ink-800 border border-ink-600 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-8 py-6 border-b border-ink-500 flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-sand-50">Recent Vendor Quotes</h2>
                <p className="text-xs text-sand-300 mt-0.5">Manage and review incoming vendor proposals</p>
              </div>
            ...
            {loading ? (
              <div className="p-8 space-y-4">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-16 bg-ink-950 animate-pulse rounded-xl" />
                ))}
              </div>
            ) : recentQuotes.length === 0 ? (
              <div className="p-16 text-center">
                <div className="w-16 h-16 bg-ink-950 rounded-full flex items-center justify-center mx-auto mb-8">
                  <MessageSquareQuote size={24} className="text-sand-500" />
                </div>
                <p className="text-sand-400 font-medium">No recent quotes found.</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {recentQuotes.map((quote) => (
                  <div key={quote.id} className="px-8 py-5 flex items-center justify-between hover:bg-ink-950 transition-colors group">
                    <div className="flex items-center gap-10">
                      <div className="w-12 h-12 rounded-xl bg-ink-900 flex items-center justify-center text-sand-200 font-bold text-lg border border-ink-600">
                        {(quote.event?.client?.name || quote.clientName || "?").charAt(0)}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-sand-50 font-bold">{quote.event?.client?.name || quote.clientName}</p>
                        ...
                        <p className="text-xs text-sand-300">For <span className="font-semibold text-sand-100">{quote.vendor?.name}</span></p>
                      </div>
                    </div>
          ...
          <div className="bg-ink-800 border border-ink-600 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-8 py-6 border-b border-ink-500 flex items-center justify-between">
              <h2 className="text-base font-bold text-sand-50">Latest Inquiries</h2>
            ...
            {loading ? (
              <div className="p-8 space-y-4">
                {[1, 2].map(i => (
                  <div key={i} className="h-12 bg-ink-950 animate-pulse rounded-lg" />
                ))}
              </div>
            ) : recentInquiries.length === 0 ? (
              <div className="p-12 text-center text-sand-400 font-medium">No new inquiries.</div>
            ) : (
              <div className="divide-y divide-slate-100">
                {recentInquiries.map((inq) => (
                  <div key={inq.id} className="px-8 py-4 flex items-center justify-between hover:bg-ink-950 transition-colors">
                    <div>
                      <p className="text-sand-50 font-bold text-sm">{inq.name}</p>
                      <p className="text-xs text-sand-300 mt-0.5">{inq.email}</p>
                    </div>
                    <p className="text-[10px] font-bold text-sand-400 uppercase tracking-wider">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-ink-800 border border-ink-600 rounded-2xl p-8 shadow-sm">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h2 className="text-lg font-bold text-sand-50 tracking-tight">Quick Actions</h2>
            </div>
            
            <div className="grid grid-cols-1 gap-3">
              <button 
                onClick={() => setIsInquiryModalOpen(true)}
                className="flex items-center gap-3 w-full p-4 bg-ink-950 hover:bg-gold-50 border border-ink-500 hover:border-gold-200 rounded-xl transition-all group"
              >
                <div className="p-2 bg-ink-800 rounded-lg shadow-sm group-hover:bg-gold-500 group-hover:text-white transition-colors">
                  <PlusCircle size={18} />
                </div>
                <span className="text-xs font-bold text-sand-100">New Inquiry</span>
              </button>

              <Link 
                href="/admin/events/new"
                className="flex items-center gap-3 w-full p-4 bg-ink-950 hover:bg-blue-50 border border-ink-500 hover:border-blue-200 rounded-xl transition-all group"
              >
                <div className="p-2 bg-ink-800 rounded-lg shadow-sm group-hover:bg-blue-500 group-hover:text-white transition-colors">
                  <Calendar size={18} />
                </div>
                <span className="text-xs font-bold text-sand-100">Add Event</span>
              </Link>

              <Link 
                href="/admin/inquiries"
                className="flex items-center gap-3 w-full p-4 bg-ink-950 hover:bg-purple-50 border border-ink-500 hover:border-purple-200 rounded-xl transition-all group"
              >
                <div className="p-2 bg-ink-800 rounded-lg shadow-sm group-hover:bg-purple-500 group-hover:text-white transition-colors">
                  <Users size={18} />
                </div>
                <span className="text-xs font-bold text-sand-100">View All Leads</span>
              </Link>

              <button 
                onClick={handleExport}
                className="flex items-center gap-3 w-full p-4 bg-ink-950 hover:bg-emerald-50 border border-ink-500 hover:border-emerald-200 rounded-xl transition-all group"
              >
                <div className="p-2 bg-ink-800 rounded-lg shadow-sm group-hover:bg-emerald-500 group-hover:text-white transition-colors">
                  <Download size={18} />
                </div>
                <span className="text-xs font-bold text-sand-100">Export Report</span>
              </button>
            </div>

            <div className="mt-12 space-y-6">
              <h3 className="text-[10px] font-black text-sand-400 uppercase tracking-[0.2em]">Recent Activity</h3>
              <div className="space-y-6">
                {activityLogs.map((log) => (
                  <div key={log.id} className="relative pl-6 group">
                    <div className="absolute left-0 top-2 w-1.5 h-1.5 rounded-full bg-ink-800 group-hover:bg-gold-500 transition-colors" />
                    <p className="text-xs text-sand-50 font-bold mb-0.5">{log.action}</p>
                    <p className="text-[10px] text-sand-400 font-medium">
                ...
                {activityLogs.length === 0 && !loading && (
                  <div className="flex flex-col items-center py-8 text-center bg-ink-950 rounded-xl border border-dashed border-ink-600">
                    <AlertCircle size={24} className="text-sand-500 mb-2" />
                    <p className="text-[10px] text-sand-400 font-bold uppercase tracking-widest leading-relaxed px-4">
      ...
      {isInquiryModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <motion.div 
            ...
            className="bg-ink-800 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden"
          >
            <div className="p-8 border-b border-ink-500 flex items-center justify-between">
              <h3 className="text-xl font-bold text-sand-50 tracking-tight">Add New Inquiry</h3>
              <button onClick={() => setIsInquiryModalOpen(false)} className="text-sand-400 hover:text-sand-200 transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleAddInquiry} className="p-8 space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-sand-400 uppercase tracking-widest ml-1">Full Name</label>
                <input 
                  type="text" 
                  required 
                  className="w-full bg-ink-950 border border-ink-600 rounded-xl p-4 text-sm focus:outline-none focus:border-gold-500 transition-all"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-sand-400 uppercase tracking-widest ml-1">Phone Number</label>
                <input 
                  type="tel" 
                  required 
                  className="w-full bg-ink-950 border border-ink-600 rounded-xl p-4 text-sm focus:outline-none focus:border-gold-500 transition-all"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-sand-400 uppercase tracking-widest ml-1">Event Type</label>
                <select 
                  required 
                  className="w-full bg-ink-950 border border-ink-600 rounded-xl p-4 text-sm focus:outline-none focus:border-gold-500 transition-all appearance-none"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-sand-400 uppercase tracking-widest ml-1">Admin Notes</label>
                <textarea 
                  rows={3}
                  className="w-full bg-ink-950 border border-ink-600 rounded-xl p-4 text-sm focus:outline-none focus:border-gold-500 transition-all resize-none"
