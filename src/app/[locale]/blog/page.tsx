    ...
    <main className="min-h-screen bg-ink-950 overflow-hidden">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-sand-300 mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-display font-medium text-sand-50 mb-8 leading-[1.1] uppercase">
              Inspire Your <span className="text-shimmer italic font-semibold">Next Event</span>
            </h1>
            <p className="text-sand-200 text-sm md:text-base font-light leading-relaxed max-w-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="flex flex-wrap gap-2 mt-12">
            {blogCategories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2.5 text-xs uppercase tracking-[0.15em] font-semibold transition-all duration-300 border ${activeCategory === cat ? "bg-gold-400 text-sand-50 border-gold-400 shadow-sm" : "border-ink-600 text-sand-200 hover:text-sand-50 hover:border-ink-500 bg-ink-800"}`}>
                {cat}
              </button>
            ))}
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.4 }} className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              {featuredPosts.map((post, idx) => (
                <Link key={post.slug} href={`/blog/${post.slug}`}>
                  <motion.article initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 + idx * 0.15 }} onMouseEnter={() => setHoveredPost(post.slug)} onMouseLeave={() => setHoveredPost(null)} className="group relative h-[480px] overflow-hidden border border-ink-600 bg-ink-800 shadow-sm hover:shadow-md rounded-sm">
                    <div className="absolute inset-0">
                    ...
                    <div className="absolute top-6 left-6 z-10">
                      <span className="px-3 py-1.5 bg-gold-400 text-sand-50 text-[10px] uppercase tracking-[0.2em] font-bold rounded-sm">Featured</span>
                    </div>
                    <div className="absolute bottom-0 left-0 right-0 p-8 z-10">
                      ...
                      <p className="text-slate-200 text-xs font-light line-clamp-2 mb-8 leading-relaxed">{post.excerpt}</p>
                      <div className="flex items-center gap-10 text-[11px] text-sand-500 uppercase tracking-wider">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-10">
            <div className="max-w-xl">
              <span className="text-[10px] uppercase tracking-[0.4em] text-gold-600 font-semibold mb-8 block">The Collection</span>
              <h2 className="text-2xl md:text-4xl font-display font-medium text-sand-50 leading-tight uppercase">Latest <span className="text-gold-600 italic">Insights</span> & Lifestyle</h2>
            </div>
            {/* Category selection refined */}
            <div className="flex flex-wrap gap-x-8 gap-y-4 border-b border-ink-600 pb-4">
              {blogCategories.map((cat) => (
                <button
                  key={cat}
                  ...
                  className={`text-[11px] uppercase tracking-[0.2em] transition-all duration-300 relative pb-2 ${
                    activeCategory === cat
                      ...
                      : "text-sand-400 hover:text-sand-50"
                  }`}
                >
                  {cat}
                  {activeCategory === cat && (
                ...
                return (
                  <Link key={post.slug} href={`/blog/${post.slug}`} className={`${colSpan} ${rowSpan} group`}>
                    <motion.article
                      ...
                      className="flex flex-col h-full bg-ink-800 border border-ink-600 rounded-sm overflow-hidden shadow-sm hover:shadow-md transition-all duration-300"
                    >
                      {/* Image - Minimal frame */}
                      <div className={`relative overflow-hidden bg-ink-900 ${isLarge ? "aspect-[16/10]" : "aspect-[4/5]"}`}>
                        <Image
                          src={post.image}
                          alt={post.title}
                      ...
                      <div className="pt-8 px-6 pb-6 flex flex-col flex-1">
                        <div className="flex items-center gap-10 mb-8">
                           <span className="text-[10px] uppercase tracking-[0.2em] text-sand-300 font-medium">{post.date}</span>
                           <div className="w-8 h-px bg-ink-800" />
                           <span className="text-[10px] uppercase tracking-[0.2em] text-sand-300 font-medium">{post.readTime}</span>
                        </div>
                        
                        <h3 className={`${isLarge ? "text-2xl md:text-3xl" : "text-lg"} font-display font-medium text-sand-50 mb-8 group-hover:text-gold-600 transition-colors duration-500 leading-[1.2] uppercase`}>
                          {post.title}
                        </h3>
                        
                        <p className="text-sand-200 text-xs font-light leading-relaxed mb-8 line-clamp-2 max-w-lg">
                          {post.excerpt}
                        </p>

                        <div className="mt-auto flex items-center gap-3 text-sand-50 text-[10px] uppercase tracking-[0.3em] font-semibold overflow-hidden">
          ...
          {filteredPosts.length === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-28">
              <p className="text-sand-300 text-sm">No articles found in this category yet. Check back soon!</p>
            </motion.div>
          )}
        </div>
      </section>
      ...
      {/* CTA */}
      <section className="py-28 border-t border-ink-600">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative overflow-hidden border border-emerald-900 bg-emerald-950 p-12 md:p-16 text-center shadow-xl rounded-sm">
            ...
            <div className="relative z-10">
              <span className="text-xs uppercase tracking-[0.25em] text-gold-400 font-semibold mb-8 block">Ready to Create Something Extraordinary?</span>
              <h2 className="text-2xl md:text-4xl font-display font-medium text-white mb-8 uppercase">
