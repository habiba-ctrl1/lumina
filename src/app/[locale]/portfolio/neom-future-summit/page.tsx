    ...
    <main className="min-h-screen bg-white overflow-hidden pt-20">
      ...
      <section className="relative h-[70vh] flex items-center justify-center bg-neutral-900">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto pt-20">
          ...
          <p className="text-neutral-300 text-lg font-light">{isAr ? "هندسة مستقبل الاستثمار العالمي في قلب الصحراء." : "Architecting the future of global investment in the heart of the desert."}</p>
        </div>
      </section>
      ...
      <section className="py-32 bg-white border border-neutral-200/80 relative z-20 -mt-10 mx-4 md:mx-auto max-w-6xl rounded-3xl shadow-2xl">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 px-8">
          {(isAr
            ? [
            { icon: Globe, label: "النطاق", val: "مستثمرون عالميون" },
            { icon: Cpu, label: "التقنية", val: "مدمجة بالذكاء الاصطناعي" },
            { icon: Zap, label: "الحجم", val: "+500 من كبار الشخصيات" },
            { icon: Building2, label: "القاعة", val: "مركز نيوم التقني" },
              ]
            : [
      ...
      <section className="py-32 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-neutral-600 font-light text-lg leading-relaxed space-y-16">
        ...
        <div>
          ...
          <p>
            {isAr
              ? "تطلّب المشروع إعداد صوت وصورة بزمن استجابة صفري، وترجمة فورية بـ12 لغة، وبصمة كربونية محايدة وفق بروتوكولات الاستدامة في نيوم."
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-2xl md:text-3xl font-sans font-bold text-neutral-900 mb-8">{isAr ? "التنفيذ" : <>The <span className="text-[var(--primary)]">Execution</span></>}</h2>
            <p className="mb-8">
              {isAr
                ? "صمّمنا «قاعة اجتماعات حيّة» غامرة بزاوية 360 درجة بجدران LED حركية تستجيب لتصورات البيانات اللحظية. وأدار فريقنا كل جانب من استئجار الطائرات الخاصة إلى تسجيل الدخول البيومتري المخصّص."
            ...
            <ul className="space-y-3 mt-6">
              {(isAr
                ? [
                "تطبيقات تجربة ضيوف مدعومة بالذكاء الاصطناعي",
                "عروض كلمات رئيسية هولوغرافية",
                "تموين مستدام من «الأرض إلى المائدة»",
                "تشفير شامل لكل الاتصالات",
                  ]
                : [
      ...
      <section className="py-32 bg-white text-center">
        <h2 className="text-2xl md:text-3xl font-sans font-bold text-neutral-900 mb-8">{isAr ? <>استضف <span className="text-[var(--primary)]">إنجازك العالمي</span></> : <>Host Your <span className="text-[var(--primary)]">Global Milestone</span></>}</h2>
        <Link
          href={`${arHref}/contact`}
        ...
        >
          {isAr ? "اطلب عرضًا مؤسسيًا" : "Request a Corporate Deck"}
        </Link>
      </section>
